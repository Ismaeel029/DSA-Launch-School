...
=>ANCHOR:
    ...
    -moves only when the runner element and the anchor element are different
    -replaces the element of the new anchor position with the runner element
=>RUNNER:
    -starts at the beginning of the array
    -moves throughout the array
    - stop for replacement when the anchor value equals the runner value
=> End the loop when the runner equals or exceed the array length
=> Return the anchor+1 for the number of distinct element in the array
*/
...
function compressToDistinct(arr){
    ...
    while(runner < N){
        if(arr[anchor] !== arr[runner]){
            anchor++;
            //replace the array element at the new anchor position
...
console.log(compressToDistinct([3, 3, 5, 7, 7, 8]));